...
const connection = {
  host: REDIS_HOST,
  port: REDIS_PORT,
  // lazyConnect: не подключаться при инструировании Queue — только при первом
  // реальном вызове. Важно в Next.js, где модуль может загружаться при
  // сборке/SSR без доступного Redis.
  lazyConnect: true,
...
// queueEvents удалён в дашборд-копии: не нужен для API routes, и создавал
// собственное подключение к Redis при импорте модуля.
